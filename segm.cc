...
int SegmenterMS::subsample(float *Xmean )
{
  ...
  //printf("index = %d, n_remain: %d\n", abs((int(float(n_remain)*float(rand())/float(SHRT_MAX))) % n_remain), n_remain);
  if(auto_segm) {
       // festr: this can reference memory out of the bounds  gen_remain[int(float(n_remain)*float(rand())/float(SHRT_MAX))]; 
    i0 = J[my_contor] = gen_remain[ abs(int(float(n_remain)*float(rand())/float(SHRT_MAX)) % n_remain) ];
  } else {
